...
const getCurrentRoomId: RoomIdGetter = () =>
    getCurrentRoomId.id ||
...
async function sendActionMessageToChat(
    ...
    domainOrPrId: string | number
): Promise<void> {
    const messageToSend = `!!/${
        messageType === 'blacklist' ? messageType + '-website' : messageType
    }- ${domainOrPrId}`.replace('approve-', 'approve'); // no need for approve to have a dash
    const userFkey =
    document.querySelector<HTMLInputElement>('input[name="fkey"]')?.value;
    ...
    if (chatNewMessageCall.status !== 200)
        throw new Error(
            `Failed to send message to chat. Returned error is ${chatNewMessageCall.status}`
        );
...
function updateWatchesAndBlacklists(parsedContent: Document): void {
    if (
        !/SmokeDetector: Auto (?:un)?(?:watch|blacklist) of/.exec(
            parsedContent.querySelector('body')?.innerText || ''
        )
    )
        return;
    try {
        const newRegex = new RegExp(
            parsedContent.querySelectorAll('code')[1].innerHTML
        );
        ...
        const isUnblacklist = Boolean(
            /Auto\sunblacklist\sof/.exec(anchorInnerHtml)
        );

        if (isWatch) {
            Domains.watchedWebsitesRegexes.push(newRegex);
        } else if (isBlacklist) {
            Domains.watchedWebsitesRegexes = Domains.watchedWebsitesRegexes.filter(
                regex => regex.toString() !== newRegex.toString()
            );
            ...
            Domains.blacklistedWebsitesRegexes =
        Domains.blacklistedWebsitesRegexes.filter(
            regex => regex.toString() !== newRegex.toString()
        );
        }
    } catch (error) {
...
export async function newChatEventOccurred({
    event_type,
    user_id,
    content,
